multilabel_dice_coefficient <- custom_metric("multilabel_dice_coefficient", function( y_true, y_pred )
{
  dimensionality <- 3L
  smoothingFactor <- 0.1
  ...
  f1_score1 <- ( 2 * unionOverlap + smoothingFactor ) /
    ( 1 + unionOverlap + smoothingFactor )
  ...
  f1_score2 <- ( 2 * unionOverlap + smoothingFactor ) /
    ( 1 + unionOverlap + smoothingFactor )
  ...
  f1_score3 <- ( 2 * unionOverlap + smoothingFactor ) /
    ( 1 + unionOverlap + smoothingFactor )
